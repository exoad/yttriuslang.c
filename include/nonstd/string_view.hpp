...
#define nssv_SUPPRESS_MSVC_WARNING(code, descr) __pragma(warning(suppress \
                                                                 : code))
#define nssv_DISABLE_MSVC_WARNINGS(codes) __pragma(warning(push)) __pragma(warning(disable \
                                                                                   : codes))
#else
...
#undef min
      ...
      nssv_constexpr14 int compare(basic_string_view other) const nssv_noexcept // (1)
      {
        ...
        return size() == other.size() ? 0 : size() < other.size() ? -1
                                                                  : 1;
      }
      ...
      {
        return assert(v.size() == 0 || v.data() != nssv_nullptr), pos >= size()
                                                                      ? npos
