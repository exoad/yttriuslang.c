...
module.exports = {
    ...
    try {
        ...
        const body = await superagent.get('https://api.mathjs.org/v4/?expr='+toCalc.replace('+', '%2B').replace('/', '%'));
        if(!toCalc || toCalc == undefined) {
            ...
            .addField("Accepted Parameters for [equation]:",  "`operation(values)` => **Operation** factor can be `sqrt`, `add`, `divide`, etc.\n**Values** are numbers to be altered and separated by `,`")
            .addField("Example", "`"+config.prefix+"element Hydrogen`\n**Parameters Taken:** `element_fullname`")
        ...
        console.log(err)
        bot.channels.cache.get(chnl.errors).send(`<@709776016314204283>\n \`\`\`console\n${err.stack}\n\`\`\``)
        const embed = new MessageEmbed()
        .setTitle("Aw snap, an internal error!")
        .setThumbnail(resource.aw_snap)
        .setDescription("Something went wrong with retrieving the info! This issue will be sent to the developer, so please hold on :)")
        .setColor(colors.error)
